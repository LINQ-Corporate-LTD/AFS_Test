...
def _get_url(drive_filename: str, source_dir, base_url: str, upload_api: str, dry_run: bool):
    """
    Resolve a file URL:
      - If drive_filename exists in source_dir (freshly downloaded from Drive) -> upload via API.
      - Else if media{drive_filename} already exists in MEDIA_ROOT             -> use that URL.
      - Otherwise returns (None, False).
    Drive files take priority so a fresh event's logo/media always replaces any
    leftover file from a previous event in MEDIA_ROOT.
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--news-prefix",   default="LDZ",
                            help="Filename prefix for news images (e.g. AFS → AFS_title.jpg)")
    ...
    def handle(self, *args, **options):
        ...
        self._upload_sheet2_files(sheet2_rows, source_dir, base_url, upload_api, dry_run)
    ...
    def _upload_sheet2_files(self, rows, source_dir, base_url, upload_api, dry_run):
        self.stdout.write("\n--- Sheet 2 Media Files ---")
