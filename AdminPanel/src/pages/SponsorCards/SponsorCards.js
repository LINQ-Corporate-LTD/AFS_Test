...
import EditSponsorCard from "./EditSponsorCard";
...
const SponsorCards = () => {
  ...
  const [sponsorCardList, setSponsorCardList] = useState([]);
  const [addSponsorCardModal, setAddSponsorCardModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editSponsorCardModal, setEditSponsorCardModal] =
    useState(false);
  ...
  useEffect(() => {
    callSponsorCardListApi();
    // eslint-disable-next-line
  }, []);

  const callSponsorCardListApi = () => {
    setloading(true);
    ...
    fetch(`http://127.0.0.1:8000/admin1/sponsorcards`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setSponsorCardList(data["sponsorCardsList"]);
  ...
  const toggleUser = useCallback(() => {
    setAddSponsorCardModal(!addSponsorCardModal);
  }, [addSponsorCardModal]);

  const isEditBtnClick = useCallback((row) => {
    setselectedRow(row);
    setEditSponsorCardModal(true);
  }, []);
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        "http://127.0.0.1:8000/admin1/deletesponsorcard",
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
          ...
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                  ...
                  <div className="d-flex align-items-center">
                    ...
                    <div className="flex-shrink-0">
                      <button
                        ...
                        onClick={toggleUser}
                      >
                        <i className="ri-add-line align-bottom me-1"></i> Add Sponsor
                        Card
                      </button>
                    </div>
                    {/* )} */}
                ...
                <CardBody>
                  <div className="mt-3">
                    {sponsorCardList && sponsorCardList?.length > 0 ? (
                      <TableContainer
                        columns={sponsorCardCol}
                        data={sponsorCardList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callSponsorCardListApi}
                        customPageSize={10}
      ...
      {addSponsorCardModal ? (
        <AddSponsorCard
          addSponsorCardModal={addSponsorCardModal}
          onCloseModal={() => setAddSponsorCardModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callSponsorCardListApi();
            setAddSponsorCardModal(false);
          }}
        />
      ) : null}
      ...
      {editSponsorCardModal ? (
        <EditSponsorCard
          row={selectedRow}
          editSponsorCardModal={editSponsorCardModal}
          onCloseModal={() => setEditSponsorCardModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callSponsorCardListApi();
            setEditSponsorCardModal(false);
          }}
        />
      ) : null}
